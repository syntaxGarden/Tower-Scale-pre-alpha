...
{	
	private void OnPronounListItemSelected(int n) {
		...
		speech_node.Text += P.X(0,true) + P.PS(false,true) + " escaped containment. Somebody go after " + P.X(1) + ", " + P.X(2) + " equipment is still secure but " + P.X(0) + P.PS(true,true) + " on " + P.X(2) + " way.\n\n" + P.PS(false,false,true) + " " + P.X(0) + " awakened yet? Those powers aren't even " + P.X(3) + ", " + P.X(0) + " " + P.PS(false) + " no right to use them.\n\n" + P.X(0,true) + P.PS(false,true) + " gotten " + P.X(4) + " into a heap of trouble now. How stupid can " + P.X(0) + " possibly be?";
	}
