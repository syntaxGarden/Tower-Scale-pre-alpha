...
public partial class Player : CharacterBody3D
{
	...
	const double DODGE_DURATION = 0.2; //The length, in seconds that a dodge lasts
	...
	//~~~~~~Slowdown variables~~~~~~
	const double SLOW_DURATION = 1; //Amount of slowdown allowed.
	...
	public override void _Input(InputEvent @event)
    {
        if (!paused)  //This way, if the game is paused, the player cannot turn
		{
			...
			if (motion != null) 
			{
				...
				//GD.Print("head.rotation = " + head.RotationDegrees + "\nself.rotation = " + RotationDegrees);
			} 
		}
    }
	...
	private void slow_code(double delta)
	{
		...
		((AnimationPlayer)GetNode("player_anim")).SpeedScale = (float)(slow_active ? SLOW_VALUE : 1);
	}
			...
			else 
			{
				if (grapple_seen != null /*TODO -	GRAPPLE COOLDOWN*/) //If a new grapple point is seen and not on cooldown
				{
					switch (player_state) 
					{
						...
						case StatesEnum.Dodging: StopDodge(); break;
	...
	private Vector3 ActualMovements(Vector3 vel, double delta) 
	{
		switch (player_state)
		{
			...
			case StatesEnum.Dodging:
				if (GetTime() < dodge_start + DODGE_DURATION ) { return dodge_dir; } //Still dodging
				else { 
					StopDodge(); 
					if (IsJumpJustPressed() && WasOnFloor()) 
					{ 
						double dodge_speed_jump_reduce = DODGE_SPEED * 0.5;
						StartFall(vel.Normalized() * (float)dodge_speed_jump_reduce);
					} 
					else if (IsSlideJustPressed() && WasOnFloor())
					{
						double dodge_speed_slide_reduce = 0.5;
						vel = vel.Normalized() * (float)dodge_speed_slide_reduce ;
						StartSlide(); //TODO - Test this (end of dodge, start sliding)
					}
					break; 
				}
			...
			case StatesEnum.Sliding: 
				...
				{
					slide_change = slide_dir.Normalized() * (float)(FRICTION * -0.1 * delta); //TODO - 	SLOPED FLOORS 
		...
		if (player_state == StatesEnum.Falling) 
		{
			...
			vel += move_dir * (float)(SPEED * FALL_MOVE_FACTOR * delta); //FALL_MOVE_FACTOR give the player more mid-air control with the movement keys	
			if (H_vel() > fall_max) 
			{ 
		...
		{
			double LERP_ACCEL = 15;
			double LERP_FRICTION = 15;
			double coef = move_dir.IsZeroApprox() ? LERP_FRICTION : LERP_ACCEL;
			vel.X = Mathf.Lerp(vel.X, move_dir.X * (float)SPEED, (float)(coef * delta));
			vel.Z = Mathf.Lerp(vel.Z, move_dir.Z * (float)SPEED, (float)(coef * delta));
			//TODO - Dodging and jumping doesn't do what it feels like it should, but there's no clear solution
			//Jumping after dodging doesn't have the speed. 
			//If jumping after dodging forces the falling state, then the jump is way too fast
			//It feels like lowering the speed that falling state starts with would MAYBE fix it, but a more natural velocity system would be better 
		}
	...
	private void StopFall() { fall_max = 0; } //Does not 
	private Vector3 StartGrapple() 
	{
		...
		grapple_vel = (grapple_seen.Position - Position).Normalized() * (float)GRAPPLE_SPEED; //TODO -	CONSERVE MOMENTUM INTO GRAPPLE
		return grapple_vel;
	}
	...
	private Vector3 StartSlide() 
	{
		...
		//TODO - 	CONSERVE MOMENTUM. Be able to fall into a downward sloping slide.

		//TODO - Remember that if the player slides to the side on a slope, that they should turn around as they slide and slide horizontally while they turn 
	...
	public override void _PhysicsProcess(double delta)
    {
		...
		MoveAndSlide(); //TODO - Need to fix this so that smacking into a wall loses momentum
		//GD.Print("vel = " + vel);
    }
